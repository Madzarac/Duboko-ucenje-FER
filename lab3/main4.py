...
def train(model, data_loader, optimizer, criterion, device, grad_clip):
    model.train()
    total_loss = 0
...
def main(args):
    ...
    model = RNNModel(rnn_type=args.rnn_type, vocab_size=len(text_vocab.stoi), embedding_dim=300, hidden_dim=args.hidden_dim,
            num_layers=args.num_layers, padding_idx=text_vocab.stoi["<PAD>"], embeddings=embedding, freeze=True, bidirectional=args.bidirectional, dropout=args.dropout).to(device)

    criterion = nn.BCEWithLogitsLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)

    for epoch in range(1, args.epochs + 1):
        train_loss = train(model, train_loader, optimizer, criterion, device, args.grad_clip)
    ...
    print("\nTest performance:")
    print(f" test loss = {test_loss:.4f}, accuracy = {test_acc*100:.2f}%, f1 = {test_f1:.4f}")
    #print(f" confusion matrix:\n{test_cm}")
